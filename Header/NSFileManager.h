...
//参考自:http://www.jianshu.com/p/b026bab2d1e7
 ...
 */
//这个方法开始卸载指定url的卷,如果卷被加密了,那么它在被卸载后重新上锁,这个完成回调block将在卸载完成后执行,如果操作成功,这个回调block将回调的值将会是nil,如果不是,那么回调的值会是一个解释为什么操作失败的error对象
 ...
 */
//返回标示directory entries的NSURL数组,如果方法返回nil,那么error将会有值.如果这个directory没有entries,那么这个方法将会返回一个空数组.最后结果还得匹配keys数组
...
//这个方法类似于NSSearchPathForDirectoriesInDomains(),返回的时url实例对象数组.这个api适合搜索文件或文件们存在指定的domains中
...
//获取a system directory and item两者之间关系
