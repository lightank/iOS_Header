...
//嵌套的.在animations和transitions时设置去通知接收者暂停触摸相关的事件的处理
...
//The value of this property is YES if the receiver is ignoring interaction events and if the nested beginIgnoringInteractionEvents and endIgnoringInteractionEvents calls are at least one level deep; otherwise, NO.
//如果有忽略一个触摸事件,那么就是yes
...
//设置摇动手势的时候，是否展示undo-redo界面,默认是YES
...
//程序后台后被杀死前,必须运行的时间;如果app使用-beginBackgroundTaskWithExpirationHandler:方法执行了一个或多个长时间任务,然后进入了后台,那么这个属性将会被调整去反映app剩余的运行时间
